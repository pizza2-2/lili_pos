...
# Rule: conditional-compilation.md
...
任务是否完成，以代码修改是否满足需求为准；运行时测试由用户自行执行。

**DoD 检查清单:**

* [ ] 代码已保存。
* [ ] 修改范围保持最小必要。
* [ ] 未引入仅用于调试的临时 `console.log` 或断点（保留必要的业务日志）。
* [ ] 已明确告知用户需要自行运行、测试并确认功能表现。
...
2. **盲目自信**: 禁止在未由用户测试确认的情况下声称“已通过运行验证”。
...
2. **修改**: 只做最小必要修改。
3. **验证**:
* 代码修改完成后，明确告知用户自行使用 HBuilderX 编译、运行、查看日志并测试功能。
* 未实际运行时，必须说明“未运行验证”。


## 5. 经验教训记录 (Strict)

* **前提**: 只有当用户反馈运行验证通过后，才允许总结经验。
* **内容**: 记录那些由用户运行验证确认的坑、配置参数或特殊处理逻辑。
...
# Rule: uni-app-x-best-practices.md
