...
function uploadSingleMediaFile(item: MediaBatchUploadItem): Promise<UTSJSONObject> {
	return new Promise((resolve, reject) => {
		...
		const headers = buildUploadHeaders()
		const uploadTimeout = timeOut < 120000 ? 120000 : timeOut
		console.log('media upload start:', baseUrl + '/api/media/files/', resolvedFilePath)
		try {
			uni.uploadFile({
				...
				timeout: uploadTimeout,
				success: (res : UploadFileSuccess) => {
					console.log('media upload success:', resolvedFilePath, res.statusCode)
					try {
						resolve(parseUploadResponseText(res.data))
					} catch (error) {
						console.log('media upload parse fail:', resolvedFilePath)
						reject(error)
					}
				},
				fail: (err : UploadFileFail) => {
					const failMessage = buildUploadFailMessage(err)
					console.log('media upload fail:', resolvedFilePath, failMessage, err.errCode)
					reject(new Error(failMessage))
				},
			})
		} catch (error) {
			console.log('media upload throw:', resolvedFilePath, stringValue((error as Error).message))
			reject(error)
		}
	})
}
