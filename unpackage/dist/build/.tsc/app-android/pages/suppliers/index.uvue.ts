...
function batchActionCandidatesFromSupplier(item: SupplierItem) : string[] {
	...
	const rawItem = rawItemText == null || rawItemText == '' ? null : JSON.parseObject<UTSJSONObject>(rawItemText)
	if (rawItem == null) {
		return []
	}
...
function consumeSupplierListRefreshNeeded(): boolean {
	const storedValue = uni.getStorageSync(supplierListRefreshStorageKey)
	if (storedValue == null) {
		return false
	}
	...
	uni.removeStorageSync(supplierListRefreshStorageKey)
	return true
}
  ...
  return _cE("view", _uM({ class: "page" }), [
    _cV(_component_lili_universal_filter, _uM({
      ...
      "filter-panel": withSlotCtx((): any[] => [
        _cE("view", _uM({ class: "supplier-filter-panel" }), [
          _cE("view", _uM({ class: "supplier-filter-actions" }), [
            _cE("view", _uM({
              class: "supplier-filter-btn supplier-filter-btn-light",
              onClick: handleFilterReset
            }), [
              _cE("text", _uM({ class: "supplier-filter-btn-light-text" }), "重置")
            ]),
            _cE("view", _uM({
              class: "supplier-filter-btn supplier-filter-btn-primary",
              onClick: applySelectedSupplier
            }), [
              _cE("text", _uM({ class: "supplier-filter-btn-primary-text" }), "应用")
            ])
          ]),
          isTrue(unref(filterOptionsLoading))
            ? _cE("view", _uM({
                key: 0,
                class: "supplier-filter-state"
              }), [
                _cE("text", _uM({ class: "supplier-filter-state-text" }), "筛选选项加载中...")
              ])
            : unref(filterOptionsError) != ''
              ? _cE("view", _uM({
                  ...
                  _cE("text", _uM({ class: "supplier-filter-state-text" }), _tD(unref(filterOptionsError)), 1 /* TEXT */)
                ])
              : filterDefinitions.value.length == 0
                ? _cE("view", _uM({
                    ...
                    _cE("text", _uM({ class: "supplier-filter-state-text" }), "暂无可用筛选项")
                  ])
                : _cE("scroll-view", _uM({
                    key: 3,
                    "scroll-y": "true",
                    class: "supplier-filter-scroll"
                  }), [
                    _cE(Fragment, null, RenderHelpers.renderList(filterDefinitions.value, (filter, __key, __index, _cached): any => {
                      return _cE("view", _uM({
                        ...
                        _cE("text", _uM({ class: "supplier-filter-group-title" }), _tD(filter.label), 1 /* TEXT */),
                        _cE("view", _uM({ class: "supplier-filter-options" }), [
                          _cE(Fragment, null, RenderHelpers.renderList(filter.options, (option, __key, __index, _cached): any => {
                            return _cE("view", _uM({
                              key: filter.key + '-' + option.value,
                              class: _nC(isFilterOptionSelected(filter.param, option.value) ? 'supplier-filter-option supplier-filter-option-active' : 'supplier-filter-option'),
                              onClick: () => {toggleFilterOption(filter.param, option.value)}
                            }), [
                              _cE("text", _uM({
                                class: _nC(isFilterOptionSelected(filter.param, option.value) ? 'supplier-filter-option-text supplier-filter-option-text-active' : 'supplier-filter-option-text')
