...
async function handleSave() {
	...
	try {
		await updateProductPricingFormula(formulaId.value, {
			name: nameText,
			code: codeText,
		...
		const message = takeLatestResponseMessage('价格公式保存成功')
...
function handleCancel() {
	uni.navigateBack({
		delta: 1,
		fail: () => {
			uni.reLaunch({ url: '/pages/tabbar/products' })
		},
	})
}
