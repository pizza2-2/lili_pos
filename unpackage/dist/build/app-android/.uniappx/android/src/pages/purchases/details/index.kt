    ...
    companion object {
        ...
        var setup: (__props: GenPagesPurchasesDetailsIndex) -> Any? = fun(__props): Any? {
            ...
            fun gen_detailToListItem_fn(item: PurchaseDetailItem): UTSJSONObject {
                ...
                return _uO("id" to item.id.toString(10), "rawId" to item.id.toString(10), "title" to stringValue(item.product_name, "商品 #" + item.product.toString(10)), "subtitle" to ("条码：" + stringValue(item.product_barcode, "-")), "image" to item.product_image, "images" to item.product_images, "amountText" to ("¥ " + stringValue(item.amount, "0.00")), "skuText" to stringValue(item.product_sku, "-"), "quantityText" to (item.received_quantity.toString(10) + "/" + item.quantity.toString(10) + "，剩余 " + item.remaining_quantity.toString(10)), "progressText" to (stringValue(item.receive_progress, "0") + "%"), "notesText" to stringValue(item.notes, "-"), "tags" to _uA<String>(statusText, "单价 ¥ " + stringValue(item.unit_price, "0.00")))
            }
