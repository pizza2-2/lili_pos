...
open class GenPagesPurchasesDetailsFrom : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesPurchasesDetailsFrom) -> Any? = fun(__props): Any? {
            ...
            val formSections = ref(_uA<UTSJSONObject>(_uO("key" to "base", "title" to "明细信息", "description" to "", "defaultOpen" to true, "fields" to _uA<UTSJSONObject>(_uO("key" to "product", "textKey" to "product_text", "label" to "商品", "type" to "bottomSelect", "required" to true, "title" to "选择商品", "placeholder" to "请选择商品", "searchPlaceholder" to "扫码或输入商品名/条码", "showScan" to true, "showAddAction" to true, "showEditAction" to true, "addPath" to "/pages/products/from", "editPath" to "/pages/products/from", "fetchData" to fetchProductOptions), _uO("key" to "quantity", "label" to "采购数量", "type" to "number", "required" to true, "placeholder" to "请输入采购数量"), _uO("key" to "received_quantity", "label" to "已收货数量", "type" to "number", "placeholder" to "通常由收货操作更新"), _uO("key" to "notes", "label" to "备注", "type" to "textarea", "placeholder" to "请输入备注")))))
