...
open class GenPagesProductsFrom : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesProductsFrom) -> Any? = fun(__props): Any? {
            ...
            val initialData = ref<UTSJSONObject>(_uO("sku" to "", "barcode" to "", "name_cn" to "", "name_en" to "", "name_other" to "", "description" to "", "category_id" to "", "category_text" to "", "supplier_id" to "", "supplier_name" to "", "purchase_price" to "0.00", "net_purchase_price" to "0.00", "cost_price" to "0.00", "base_sales_price" to "0.00", "status" to "ACTIVE", "is_featured" to false, "is_new" to false, "is_bestseller" to false, "sort_order" to "0", "images" to _uA<String>(), "imageItems" to _uA<UTSJSONObject>()))
            ...
            fun gen_convertCategoryTreeItems_fn(items: UTSArray<UTSJSONObject>): UTSArray<UTSJSONObject> {
                ...
                run {
                    ...
                    while(i < items.length){
                        ...
                        val option: UTSJSONObject = _uO("value" to buildOptionValue(item), "text" to label, "label" to label, "full_name" to stringValue(item["full_name"], label), "code" to stringValue(item["code"]), "level" to intValue(item["level"], -1), "parent_value" to stringValue(item["parent_value"]), "disabled" to booleanValue(item["disabled"]), "has_children" to (booleanValue(item["has_children"]) || treeChildren.length > 0), "children" to treeChildren)
                        result.push(option)
                        i++
                    }
                }
            ...
            fun gen_fetchCategoryOptions_fn(params: UTSJSONObject): UTSPromise<UTSJSONObject> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        val queryParams: UTSJSONObject = _uO("key" to "parent", "page" to intValue(pageValue, 1), "page_size" to intValue(pageSizeValue, 20))
                        if (keywordValue != "") {
