    ...
    _cV(_component_lili_universal_filter, _uM({
      ...
      "filter-panel": withSlotCtx((): any[] => [
        _cE("view", _uM({ class: "filter-panel" }), [
          ...
          _cE("text", _uM({ class: "filter-title filter-title-spaced" }), "付款状态"),
          _cE("view", _uM({ class: "filter-row" }), [
            _cE("view", _uM({
              class: _nC(unref(selectedPaymentStatus) == null ? 'filter-chip filter-chip-active' : 'filter-chip'),
              onClick: () => {selectPaymentStatus(null)}
            }), [
              _cE("text", _uM({
                class: _nC(unref(selectedPaymentStatus) == null ? 'filter-chip-text filter-chip-text-active' : 'filter-chip-text')
              }), "全部", 2 /* CLASS */)
            ], 10 /* CLASS, PROPS */, ["onClick"]),
            _cE("view", _uM({
              class: _nC(unref(selectedPaymentStatus) == 'false' ? 'filter-chip filter-chip-active' : 'filter-chip'),
              onClick: () => {selectPaymentStatus('false')}
            }), [
              _cE("text", _uM({
                class: _nC(unref(selectedPaymentStatus) == 'false' ? 'filter-chip-text filter-chip-text-active' : 'filter-chip-text')
              }), "未付款", 2 /* CLASS */)
            ], 10 /* CLASS, PROPS */, ["onClick"]),
            _cE("view", _uM({
              class: _nC(unref(selectedPaymentStatus) == 'true' ? 'filter-chip filter-chip-active' : 'filter-chip'),
              onClick: () => {selectPaymentStatus('true')}
            }), [
              _cE("text", _uM({
                class: _nC(unref(selectedPaymentStatus) == 'true' ? 'filter-chip-text filter-chip-text-active' : 'filter-chip-text')
              }), "已付款", 2 /* CLASS */)
            ], 10 /* CLASS, PROPS */, ["onClick"])
          ]),
