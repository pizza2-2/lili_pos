...
import { ProductItem, ProductMutationData, createProduct, getProductDetail, updateProduct } from '@/pkg/api/modules/products.uts'
import { batchUploadMediaFiles, MediaBatchUploadItem } from '@/pkg/api/modules/media.uts'
import { authState } from '@/store/auth'

type SelectOption = { __$originalPosition?: UTSSourceMapPosition<"SelectOption", "pages/products/from.uvue", 49, 6>;
	value: string
	text: string
}
...
function parseObjectArray(value: any | null): UTSJSONObject[] {
	...
	const parsed = UTSAndroid.consoleDebugError(JSON.parseArray<UTSJSONObject>(text), " at pages/products/from.uvue:148")
	if (parsed == null) {
		return [] as UTSJSONObject[]
	}
	return parsed!
...
function parseErrorMessage(error: any, fallback: string): string {
	...
	if (error != null) {
		...
		if (errorText != null && errorText != '') {
			const parsedError = UTSAndroid.consoleDebugError(JSON.parseObject<UTSJSONObject>(errorText), " at pages/products/from.uvue:164")
			if (parsedError != null) {
...
function buildUploadHeaders(): UTSJSONObject {
	const headers = { __$originalPosition: new UTSSourceMapPosition("headers", "pages/products/from.uvue", 180, 8), } as UTSJSONObject
	if (authState.token != '') {
...
async function fetchSupplierOptions(params: UTSJSONObject): Promise<UTSJSONObject> {
	...
	return buildSelectResponse(buildSelectOptions(raw), params)
}
...
function convertCategoryTreeItems(items: UTSJSONObject[]): UTSJSONObject[] {
	...
	for (let i = 0; i < items.length; i++) {
		...
		const option: UTSJSONObject = {__$originalPosition: new UTSSourceMapPosition("option", "pages/products/from.uvue", 321, 9),
...
async function fetchCategoryOptions(params: UTSJSONObject): Promise<UTSJSONObject> {
	...
	const queryParams = { __$originalPosition: new UTSSourceMapPosition("queryParams", "pages/products/from.uvue", 373, 8), 
		key: 'parent',
