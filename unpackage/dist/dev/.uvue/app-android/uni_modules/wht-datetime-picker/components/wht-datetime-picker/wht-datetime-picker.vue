const __sfc__ = defineComponent({
    ...
    props: {
        ...
        startYear: {
            type: Number,
            default: (): number => new Date().getFullYear() - 5
        },
        endYear: {
            type: Number,
            default: (): number => new Date().getFullYear() + 5
        ...
        quickOptions: {
            ...
            validator: function (value) {
                return value.every((option): boolean => {
                    if (!option.label)
                        return false;
    ...
    watch: {
        value: {
            ...
            handler(val) {
                if (!this.visible) {
                    if (this.isRange) {
                        this.rangeValues = Array.isArray(val) ? (val as any[]).map((v): any => this.parseDate(v)) : [new Date(), new Date()];
                    }
                    else {
        ...
        currentValue: {
            handler(val) {
                if (!Array.isArray(val) || (val as any[]).some((v): boolean => typeof v !== 'number')) {
    ...
    methods: {
        // 格式化日期
        formatDate(date, type = 'datetime'): string {
            if (!date)
                return '';
            try {
                // 确保是 Date 对象
        ...
        parseDate(value): any {
            ...
            try {
                ...
                if (value instanceof Date) {
                    date = new Date((value as Date).getTime());
                }
                    ...
                    if ((value as string).includes('T')) {
                        ...
                        const parts = (value as string).split(/[-\s:/]/).map((p): number => parseInt(p));
                        if (parts.length >= 3) {
            ...
            try {
                if (this.isRange) {
                    ...
                    if (Array.isArray(value) && (value as any[]).length === 2) {
                        this.rangeValues = (value as any[]).map((v): any => this.parseDate(v));
                    }
                    else if (typeof value === 'string') {
                        // 尝试解析字符串格式的日期
        ...
        handleConfirm() {
            try {
                if (this.isRange) {
                    ...
                    const value = this.rangeValues.map((date): Date => new Date(date.getTime()));
                    const formatted = value.map((date): string => this.formatDate(date, this.mode.replace('-range', '')));
        ...
        handleQuickSelect(option, index) {
            ...
            try {
                if (this.isRange) {
                    ...
                    this.rangeValues = rangeValue.map((v): any => this.parseDate(v));
                }
                else {
                    // 处理单个值
        ...
        validateDate(date): boolean {
            if (!(date instanceof Date) || isNaN((date as Date).getTime())) {
        ...
        updateCurrentValue() {
            ...
            const values = [];
            if (this.showYear) {
                const yearIndex = this.years.findIndex((y): boolean => parseInt(y) === date.getFullYear());
                values.push(yearIndex >= 0 ? yearIndex : 0);
            }
            if (this.showMonth) {
                const monthStr = String(date.getMonth() + 1).padStart(2, '0');
                const monthIndex = this.months.findIndex((m): boolean => m === monthStr);
                values.push(monthIndex >= 0 ? monthIndex : 0);
            }
            if (this.showDay) {
                const dayStr = String(date.getDate()).padStart(2, '0');
                const dayIndex = this.days.findIndex((d): boolean => d === dayStr);
                values.push(dayIndex >= 0 ? dayIndex : 0);
            }
            if (this.showHour) {
                const hourStr = String(date.getHours()).padStart(2, '0');
                const hourIndex = this.hours.findIndex((h): boolean => h === hourStr);
                values.push(hourIndex >= 0 ? hourIndex : 0);
            }
            if (this.showMinute) {
                const minuteStr = String(date.getMinutes()).padStart(2, '0');
                const minuteIndex = this.minutes.findIndex((m): boolean => m === minuteStr);
                values.push(minuteIndex >= 0 ? minuteIndex : 0);
            }
            if (this.showSecond) {
                const secondStr = String(date.getSeconds()).padStart(2, '0');
                const secondIndex = this.seconds.findIndex((s): boolean => s === secondStr);
...
//# sourceMappingURL=wht-datetime-picker.vue.map