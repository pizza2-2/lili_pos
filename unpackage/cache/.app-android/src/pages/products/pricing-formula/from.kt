...
open class GenPagesProductsPricingFormulaFrom : BasePage {
    constructor(__ins: ComponentInternalInstance, __renderer: String?) : super(__ins, __renderer) {}
    companion object {
        @Suppress("UNUSED_PARAMETER", "UNUSED_VARIABLE")
        var setup: (__props: GenPagesProductsPricingFormulaFrom) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesProductsPricingFormulaFrom
            ...
            val steps = ref(_uA<FormulaStep__1>())
            val testValue = ref("9.90")
            val sourceChoices = ref(_uA<FormulaChoice__1>(FormulaChoice__1(value = "VALUE", label = "不固定数据源"), FormulaChoice__1(value = "COST_PRICE", label = "成本价"), FormulaChoice__1(value = "PURCHASE_PRICE", label = "含税采购价"), FormulaChoice__1(value = "NET_PURCHASE_PRICE", label = "不含税采购价"), FormulaChoice__1(value = "BASE_SALES_PRICE", label = "基础销售价")))
            val roundModeChoices = ref(_uA<FormulaChoice__1>(FormulaChoice__1(value = "round_digits", label = "四舍五入"), FormulaChoice__1(value = "ceil_step", label = "按步长向上取整"), FormulaChoice__1(value = "floor_step", label = "按步长向下取整"), FormulaChoice__1(value = "ceil_tail_09", label = "向上取到尾数 0.9"), FormulaChoice__1(value = "floor_tail_09", label = "向下取到尾数 0.9")))
            ...
            fun gen_cloneSteps_fn(source: UTSArray<FormulaStep__1>): UTSArray<FormulaStep__1> {
                val result: UTSArray<FormulaStep__1> = _uA()
                run {
                    var i: Number = 0
                    while(i < source.length){
                        val item = source[i]
                        result.push(FormulaStep__1(kind = item.kind, operator = item.operator, operand = item.operand, mode = item.mode, parameter = item.parameter))
                        i += 1
                    }
                }
                return result
            ...
            fun gen_stepTitle_fn(step: FormulaStep__1): String {
                if (step.kind == "binary") {
                    if (step.operator == "*") {
                        return "乘法"
                    }
            ...
            fun gen_describeStep_fn(step: FormulaStep__1): String {
                if (step.kind == "binary") {
            ...
            fun gen_buildExpression_fn(baseValue: String, sourceSteps: UTSArray<FormulaStep__1>): String {
            ...
            fun gen_buildDisplayExpression_fn(baseValue: String, sourceSteps: UTSArray<FormulaStep__1>): String {
                if (sourceSteps.length == 0) {
                    return ""
                }
            ...
            fun gen_evaluateSteps_fn(baseText: String, sourceSteps: UTSArray<FormulaStep__1>): String {
            ...
            fun gen_parseExpressionNode_fn(value: String, outSteps: UTSArray<FormulaStep__1>): String {
                val text = trimOuterPair(value)
                var body = functionBody(text, "CEIL_TO_09")
                if (body != null) {
                    val base = gen_parseExpressionNode_fn(body!!, outSteps)
                    outSteps.push(FormulaStep__1(kind = "round", operator = "", operand = "", mode = "ceil_tail_09", parameter = ""))
                    return base
                }
                body = functionBody(text, "FLOOR_TO_09")
                if (body != null) {
                    val base = gen_parseExpressionNode_fn(body!!, outSteps)
                    outSteps.push(FormulaStep__1(kind = "round", operator = "", operand = "", mode = "floor_tail_09", parameter = ""))
                    return base
                }
                ...
                if (body != null) {
                    ...
                    outSteps.push(FormulaStep__1(kind = "round", operator = "", operand = "", mode = "round_digits", parameter = parameter))
                    return base
                }
                ...
                if (body != null) {
                    ...
                    outSteps.push(FormulaStep__1(kind = "round", operator = "", operand = "", mode = "ceil_step", parameter = parameter))
                    return base
                }
                ...
                if (body != null) {
                    ...
                    outSteps.push(FormulaStep__1(kind = "round", operator = "", operand = "", mode = "floor_step", parameter = parameter))
                    return base
                }
                ...
                if (opIndex >= 0) {
                    ...
                    outSteps.push(FormulaStep__1(kind = "binary", operator = operator, operand = right, mode = "", parameter = ""))
                    return base
                }
                return normalizeFormulaSource(text)
            }
            val parseExpressionNode = ::gen_parseExpressionNode_fn
            fun gen_parseExpressionToBuilder_fn(expression: String): ParsedFormula__1 {
                val parsedSteps: UTSArray<FormulaStep__1> = _uA()
                try {
                    val base = parseExpressionNode(expression.trim(), parsedSteps)
                    return ParsedFormula__1(success = true, base_variable = normalizeFormulaSource(base), steps = parsedSteps)
                }
                 catch (error: Throwable) {
                    return ParsedFormula__1(success = false, base_variable = "VALUE", steps = _uA<FormulaStep__1>())
                }
            }
            ...
            fun gen_applyFormula_fn(item: ProductPricingFormulaItem) {
                ...
                steps.value = _uA<FormulaStep__1>()
            ...
            fun gen_addBinaryStep_fn(operator: String) {
                ...
                next.push(FormulaStep__1(kind = "binary", operator = operator, operand = if (operator == "*" || operator == "/") {
                    "1"
                } else {
                    "0"
                }
            ...
            fun gen_addRoundStep_fn(mode: String) {
                ...
                val next = cloneSteps(steps.value)
                next.push(FormulaStep__1(kind = "round", operator = "", operand = "", mode = mode, parameter = parameter))
                steps.value = next
            }
            val addRoundStep = ::gen_addRoundStep_fn
            fun gen_replaceStep_fn(index: Number, nextStep: FormulaStep__1) {
                val next = cloneSteps(steps.value)
            ...
            fun gen_setStepOperator_fn(index: Number, operator: String) {
                ...
                replaceStep(index, FormulaStep__1(kind = current.kind, operator = operator, operand = current.operand, mode = current.mode, parameter = current.parameter))
            }
            ...
            fun gen_setStepRoundMode_fn(index: Number, mode: String) {
                ...
                replaceStep(index, FormulaStep__1(kind = current.kind, operator = current.operator, operand = current.operand, mode = mode, parameter = parameter))
            }
            ...
            fun gen_handleStepOperandInput_fn(index: Number, event: Any) {
                ...
                val current = steps.value[index]
                replaceStep(index, FormulaStep__1(kind = current.kind, operator = current.operator, operand = readEventValue(event), mode = current.mode, parameter = current.parameter))
            }
            ...
            fun gen_handleStepParameterInput_fn(index: Number, event: Any) {
                ...
                val current = steps.value[index]
                replaceStep(index, FormulaStep__1(kind = current.kind, operator = current.operator, operand = current.operand, mode = current.mode, parameter = readEventValue(event)))
            }
            val handleStepParameterInput = ::gen_handleStepParameterInput_fn
            fun gen_removeStep_fn(index: Number) {
                val next: UTSArray<FormulaStep__1> = _uA()
                run {
                    var i: Number = 0
            ...
            fun gen_markPricingFormulaListRefreshNeeded_fn() {
                uni_setStorageSync(pricingFormulaListRefreshStorageKey, "1")
            }
            val markPricingFormulaListRefreshNeeded = ::gen_markPricingFormulaListRefreshNeeded_fn
            fun gen_goBackToList_fn() {
                uni_navigateBack(NavigateBackOptions(delta = 1, fail = fun(_){
                    uni_navigateTo(NavigateToOptions(url = "/pages/products/pricing-formula/index"))
                }
                ))
            }
            ...
            fun gen_handleSave_fn(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        try {
                            val payload = ProductPricingFormulaMutationData(name = nameText, code = codeText, expression = expression, description = description.value.trim(), is_active = isActive.value)
                            var successText = "价格公式创建成功"
                            if (formMode.value == "edit") {
                                await(updateProductPricingFormula(formulaId.value, payload))
                                successText = "价格公式保存成功"
                            } else {
                                val created = await(createProductPricingFormula(payload))
                                formulaId.value = created.id.toString(10)
                                formMode.value = "edit"
                            }
                            val message = takeLatestResponseMessage(successText)
            ...
            onLoad(fun(event: OnLoadOptions){
                ...
                formMode.value = if (formulaId.value == "") {
                    "create"
                } else {
                    "edit"
                }
            ...
            return fun(): Any? {
                ...
                return _cE("view", _uM("class" to "page"), _uA(
                    _cV(_component_lili_universal_filter, _uM("title" to pageTitle.value, "showBack" to true, "showSearch" to false, "showHome" to true, "homePath" to "/pages/products/pricing-formula/index", "backgroundColor" to "#EEF2F7"), null, 8, _uA(
                        "title"
                    )),
                            ...
                            if (isTrue(unref(loading))) {
                                ...
                                if (unref(errorMessage) != "") {
                                    _cE("view", _uM("key" to 1, "class" to "state-card"), _uA(
                                        ...
                                        _cE("view", _uM("class" to "small-btn", "onClick" to loadFormula), _uA(
