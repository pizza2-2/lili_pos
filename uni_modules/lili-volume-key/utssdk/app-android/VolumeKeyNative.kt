package uts.sdk.modules.liliVolumeKey
...
object VolumeKeyNative {
        ...
        activity.runOnUiThread {
            try {
                ...
                if (interceptCallback != null && installedActivity === activity) {
                    if (DEBUG) console.log("lili-volume-key-native: already installed")
                ...
                if (DEBUG) console.log("lili-volume-key-native: installed callback=" + current.javaClass.name)
            } catch (error: Throwable) {
                if (DEBUG) console.log("lili-volume-key-native: install failed " + error.message)
            }
        }
        return true
    }
        ...
        activity.runOnUiThread {
            try {
                ...
                if (installedActivity === activity && interceptCallback != null && originalCallback != null) {
                    ...
                    if (DEBUG) console.log("lili-volume-key-native: uninstalled")
                }
            } catch (error: Throwable) {
                if (DEBUG) console.log("lili-volume-key-native: uninstall failed " + error.message)
            } finally {
    ...
    private fun emit(activity: Activity, event: KeyEvent) {
        ...
        if (DEBUG) console.log("lili-volume-key-native: consumed key=" + event.keyCode + " action=" + event.action)
    }
